...
var handleRenderTypeahead = function () {
	...
	function llenarCabecera() {
		vaciarCabecera();

		$('#inputCodigoCliente').val(orderData[0].CodigoCliente);

		getCustomerById(orderData[0].CodigoCliente)
			.then(function (dataCliente) {
				$('#inputCodigoCliente').attr('data-codPriceList', dataCliente.Cod_Lista_Precio);
			});


		$('#inputNombreCliente').val(orderData[0].NombreCliente);

		$('#inputPersonaContacto').append(`
			 <option value="${orderData[0].CodPersonaContacto}" selected>${orderData[0].NombrePersonaContacto}</option>
		`)

		$('#inputNumeroReferencia').val(orderData[0].NumRef);

		$('#inputDireccionDestino').append(`
			 <option value="${orderData[0].CodDireEntrega}" selected>${orderData[0].CodDireEntrega}</option>
		`)

		$('#inputDestinatarioFactura').append(`
			 <option value="${orderData[0].CodDireFactura}" selected>${orderData[0].CodDireFactura}</option>
		`)
		...
		/*FECHAS*/
		$('#datepicker-default').val(obtenerFechaSinHora(orderData[0].FechaContabilizacion));
		$('#datepicker-range').val(obtenerFechaSinHora(orderData[0].FechaEntrega));
		$('#datepicker-inline').val(obtenerFechaSinHora(orderData[0].FechaDocumento));
		/*FECHAS*/
		...
		getSerieList()
			.then(function (Series) {
				...
				Series.forEach(function (serie) {
					...
					if (!selectElement.find('option[value="' + serie.CodSerie + '"]').length) {
						selectElement.append(option);
					}
				});
				/*
...
var handleRenderTableData = function () {
	...
	class OrdenPreliminar {
		constructor(DocEntry, CardCode, ContactPersonCode, NumAtCard, ShipToCode, PayToCode, DocCurrency, DocDate
	...
	/*BEGIN PATCH nueva orden preliminar */
	...
	$('#myModal').on('click', '#btnAceptar', function () {
		...
		$('#detalleRow tr').each(function () {
			...
			var ShipDate = formatearFecha(fechaFormateada2, 'YYYY-MM-DD');
	...
	/*END PATCH nueva orden preliminar */
